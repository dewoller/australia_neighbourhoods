create table meshblock_concordance_BASE as
with pairs_mco as (
  select mc.mb_code16 as mb_code16, mb.mb_code16 as to_mb_code16, mc.buffer_BASE, mb.geom as to_mb_geom
  FROM meshblock_centroid mc
  JOIN meshblock mb
  ON ST_DWithin(mc.geom, mb.geom, BASE, false) )
...
, pairs_mco mco
, lateral (values(st_intersection( mco.buffer_BASE, st_transform( to_mb_geom, 3577)))) as s1(intersection)
...
create index meshblock_concordance_BASE_idx_1 on meshblock_concordance_BASE( mb_code16, to_mb_code16 );
create index meshblock_concordance_BASE_idx_2 on meshblock_concordance_BASE( to_mb_code16, mb_code16 );
create  table meshblock_statistics_BASE as (
  ...
  from meshblock_concordance_BASE mc
